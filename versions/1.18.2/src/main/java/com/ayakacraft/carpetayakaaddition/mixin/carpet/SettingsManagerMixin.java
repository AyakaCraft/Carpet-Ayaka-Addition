...
public class SettingsManagerMixin {
    ...
    static {
        try {
            parsedRuleConstructor = ParsedRule.class.getDeclaredConstructor(Field.class, Rule.class, SettingsManager.class);
    ...
    @Unique
    private static ParsedRule<?> of(Field field, Rule rule, SettingsManager manager) {
        try {
            return parsedRuleConstructor.newInstance(field, rule, manager);
        } catch (ReflectiveOperationException e) {
            throw new RuntimeException(e);
        }
    }
    ...
    private void injectConditions(Class<?> settingsClass, Operation<Void> original) {
        ...
        for (Field f : settingsClass.getDeclaredFields()) {
            ...
            ParsedRule<?> parsed = of(f, rule, (SettingsManager) (Object) this);
            for (Class<? extends carpet.settings.Condition> condition : rule.condition()) {
                try {
                    Constructor<? extends carpet.settings.Condition> constr = condition.getDeclaredConstructor();
